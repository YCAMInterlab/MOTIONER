/******************************************************************************************
 * Accelerometer calibration for MotionerIMU 
...
void caliAcce()
{
  ...
  fill(200);
  textSize(16);
  text(minAccel[0] + ", " + maxAccel[0], width/2, height-180, 0);
  text(minAccel[1] + ", " + maxAccel[1], width/2, height-150, 0);
  text(minAccel[2] + ", " + maxAccel[2], width/2, height-120, 0);
  textSize(16);
  ...
  pushMatrix(); 
  {
    ...
    if (serial.available() >= 36) {
      ...
      // Draw new point
      strokeWeight(5);
      stroke((accel[1] + 800) / 8, 255, 255);
      //translate(accel[0], accel[1], accel[2]);
      line(accel[0], accel[1], accel[2], paccel[0], paccel[1], paccel[2]);
      strokeWeight(1);
    }
  } 
  popMatrix();
...
void outputAcceCalibration() 
{
  /// motioner
  byte[] data = new byte[4*6]; /// float *6

  for (int i=0; i<3; i++) {
    printFloat(data, i*8+0, minAccel[i]);
    printFloat(data, i*8+4, maxAccel[i]);
  }

  //serial.clear();  // Clear input buffer up to here
  serial.write("#ca");  // Send calibrated data to IMU
  ...
  // Output calibration
  System.out.printf("Accelerometer calibration was written in EEPROM on your MotionerIMU!\n");
  System.out.printf("Accelerometer range min/max = {%.6g/%.6g, %.6g/%.6g, %.6g/%.6g};\n", minAccel[0], maxAccel[0], minAccel[1], maxAccel[1], minAccel[2], maxAccel[2]);
  println("\n");
  resynch();
}
